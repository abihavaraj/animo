-- Simplified RLS Optimization - Only for Tables That Actually Exist
-- Based on the actual policy list provided by the user

-- Drop old policies for notification_settings (multiple policies exist)
...
-- Drop old policies for client_documents (multiple policies exist)
...
-- Drop old policies for client_notes (multiple policies exist)
...
-- Drop old policies for payments (multiple policies exist)
DROP POLICY IF EXISTS "Admins can view all payments" ON payments;
DROP POLICY IF EXISTS "Allow reception and admin access to payments" ON payments;
DROP POLICY IF EXISTS "Users can view own payments" ON payments;

-- Drop old policies for manual_credits (multiple policies exist)
...
-- Drop old policies for single tables with old policies
...
-- Create optimized policies for tables that actually exist
DO $$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'client_activity_log' AND policyname = 'client_activity_log_access_policy') THEN
        CREATE POLICY "client_activity_log_access_policy" ON client_activity_log
          FOR ALL USING (
            auth.uid() = client_id OR 
            (auth.uid() = performed_by AND performed_by IS NOT NULL) OR 
            (SELECT role FROM public.users WHERE id = auth.uid()) IN ('admin', 'reception')
          );
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'client_lifecycle' AND policyname = 'client_lifecycle_access_policy') THEN
        CREATE POLICY "client_lifecycle_access_policy" ON client_lifecycle
          FOR ALL USING (
            auth.uid() = client_id OR 
            (auth.uid() = stage_changed_by AND stage_changed_by IS NOT NULL) OR 
            (SELECT role FROM public.users WHERE id = auth.uid()) IN ('admin', 'reception')
          );
    END IF;
    ...
    -- Class notification settings: Single consolidated policy (admin/reception only)
