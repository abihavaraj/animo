-- Fixed RLS Optimization with Correct Column Names
...
-- Create optimized policies for remaining tables with correct column names
DO $$
BEGIN
    ...
    -- Client notes: Single consolidated policy (uses client_id and admin_id)
    ...
    -- Client lifecycle: Single consolidated policy (uses client_id and admin_id)
...
CREATE INDEX IF NOT EXISTS idx_client_notes_client_admin ON client_notes(client_id, admin_id);
...
CREATE INDEX IF NOT EXISTS idx_client_lifecycle_client_admin ON client_lifecycle(client_id, admin_id);
